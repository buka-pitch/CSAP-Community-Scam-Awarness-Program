...
export const CreateQuestionScene = new WizardScene<MyContext>("CreateQuestionScene", async (cxt, next) => {
  ...
  await cxt.sendMessage("Which lesson do you want to add Question to ?\n", Markup.inlineKeyboard(button, { "columns": 1 }));;
  ...
  if (await cxt.message?.text) {
    ...
    let question = await createQuestion(cxt.session.lessonTitle, { "question": cxt.session.question, "answer": cxt.session.correct_answer, "choice": choice });
    cxt.reply(question);
...
export const CreateLessonScene = new WizardScene<MyContext>(
  "CreateLessonScene",
  async (cxt, next) => {
    await cxt.sendMessage("Provide the Course title that you want to add lesson for : ");
    return cxt.wizard.next();
  }, async (cxt, next) => {
    if (cxt?.message?.text) {
      cxt.scene.session.myWizardSessionProp = { course: "", title: "", description: "" }
  ...
  async (cxt, next) => {
    if (cxt.message?.text) {
      ...
      const courseId = await prisma.course.findUnique({ where: { title: cxt.scene.session.myWizardSessionProp.course } }).catch((err) => {
      ...
      if (!lesson) {
        cxt.sendMessage("Lesson Couldn't be Created1");
        return cxt.scene.leave();
      }
